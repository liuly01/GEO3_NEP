...
% File paths
allsampleFile = 'D:\han05\data\fluxdata\MM\Classification\PFT\dataset_newevi\grassland.csv';

% Load data
allsampleData = readtable(allsampleFile);
allsampleVars = table2array(allsampleData(:, [14, 27, 21, 18, 29, 22, 19]));
sampleNEE = table2array(allsampleData(:, 5));
siteNames = allsampleData{:, 1};

% Define weights for each environmental variable
weights = [0.24, 0.23, 0.16, 0.08, 0.10, 0.09, 0.1];  % grassland
...
for fold = 1:numFolds
    ...
    for i = 1:numUnpredicted
        ...
        for j = 1:numSamples
            ...
            similarityvars = zeros(1, 7);  % For each environmental variable
