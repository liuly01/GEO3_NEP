...
% File paths
allsampleFile = 'D:\han05\data\fluxdata\MM\Classification\PFT\dataset_newevi\forest.csv';

% Load data
allsampleData = readtable(allsampleFile);
allsampleVars = table2array(allsampleData(:, [27, 31, 14, 23, 20, 17, 19, 34])); % 提取环境变量
sampleNEE = table2array(allsampleData(:, 5)); % NEE 在第四列
siteNames = allsampleData{:, 1}; % 站点名称在第一列

% Define weights for each environmental variable
weights = [0.22, 0.05, 0.20, 0.16, 0.19, 0.07, 0.07, 0.04];  % forest
...
disp('Evaluation Criteria for the Third Law of Geography：');
disp(['R2: ', num2str(r2_geo_mean),'±', num2str(r2_geo_std)]);
disp(['RMSE: ', num2str(rmse_geo_mean),'±', num2str(rmse_geo_std)]);
disp(['Bias: ', num2str(bias_geo_mean),'±', num2str(bias_geo_std)]);
disp(['Slope: ', num2str(slope_geo_mean),'±', num2str(slope_geo_std)]);